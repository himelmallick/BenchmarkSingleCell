...
workingDirectory<-'/gpfs/gsfs11/users/chatterjees7/TweedieVerse/SingleCell/Splatter/ZeroInflated'
...
{
methods.all<-c("CPLM","CPLM.GMPR",'CPLM.SCRAN',
              "DESeq2","DEsingle","edgeR","limmaVOOM",
              "MAST","metagenomeSeq","monocle",
              "scRE","Wilcoxon","ZICP","ZICP.GMPR",
              'ZICP.SCRAN',"zingeR")
...
inputDirectory<-paste(workingDirectory,'Input',sep='/')
...
if(run.method=="CPLM.GMPR"){
normlist<-list.GMPR(fitlist)
}else if(run.method=="CPLM.SCRAN"){
normlist<-list.SCRAN(fitlist)
}else if(run.method=="ZICP.GMPR"){
normlist<-list.GMPR(fitlist)
...
}else if(run.method=="MAST2"){
output<-list.MAST2(fitlist,transformation=transfMethod)
...
}else if(run.method=="scRE2"){
output<-list.scRE2(fitlist,transformation=transfMethod)
...
stopCluster(cl)
outputDirectory<-paste(workingDirectory,'Outputs',sep="/")
