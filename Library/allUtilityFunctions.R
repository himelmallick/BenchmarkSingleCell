...
#################################
  ...
  for(i in 1:nrow(all.combs)){
    ...
    combinations<-paste(methodName,ZeroInflate,RandomEffect,
                        metadataType,nCells,nPerSubject, 
                        nGenes,nMetadata,effectSize,
                        dropout.mid,pDE,nIterations,sep="_")
    
    ##########################
    ### Gathering results ####
    ##########################
    
    tmp1<-data.frame(combinations=combinations,Sensitivity=Sensitivity,
                     Specificity=Specificity,FDR=FDR,FScore=FScore,
                     MCC=MCC,AUC=AUC,pAUC=pAUC,conservArea=conservArea,
                     liberalArea=liberalArea,totalArea=totalArea,time=time,
                     alpha=alpha,qval=qval,methodName=methodName,
                     ZeroInflate=ZeroInflate,RandomEffect=RandomEffect,
                     metadataType=metadataType,nCells=nCells,
                     nPerSubject=nPerSubject,nGenes=nGenes,nMetadata=nMetadata,
                     effectSize=effectSize,dropout.mid=dropout.mid,
                     pDE=pDE,nIterations=nIterations)
