...
###################################
# Fit MAST2 To A List of Datasets #
###################################

list.MAST2<-function(physeq, transformation = 'NONE', multiple_qvalues = TRUE){
  foreach(physeq = physeq, 
          .export = c("pvalueAdjustment_HM", "append_qvalues",
                      "fit.MAST2","rename.features", "get_pval_MAST"), 
          ...
          .errorhandling = "remove") %dopar% 
    {
      ...
      ID<-physeq$ID
      DD<-fit.MAST2(features, metadata, libSize, ID, transformation, multiple_qvalues)
