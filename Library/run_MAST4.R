...
##########################
# Fit MAST4 To A Dataset #
##########################

fit.MAST4 <-function(features, 
                    metadata, 
                    libSize, 
                    ID, 
                    transformation,
...
###################################
# Fit MAST4 To A List of Datasets #
###################################

list.MAST4<-function(physeq, transformation = 'NONE', multiple_qvalues = TRUE){
  foreach(physeq = physeq, 
          .export = c("pvalueAdjustment_HM", "append_qvalues",
                      "fit.MAST4","rename.features", "get_pval_MAST"), 
          ...
          .errorhandling = "remove") %dopar% 
    {
      ...
      ID<-physeq$ID
      DD<-fit.MAST4(features, metadata, libSize, ID, transformation, multiple_qvalues)
