#######################################################################################
# Zero-inflated Compound Poisson with libray size covariate in both submodels (ZICP4) #
#######################################################################################
...
fit.ZICP4 <- function(features, 
  ...
  if (transformation!='NONE') stop ('Transformation currently not supported for a default ZICP4 model. Use NONE.')
...
list.ZICP4<-function(physeq, transformation = 'NONE', multiple_qvalues = TRUE){
  foreach(physeq = physeq, 
          ...
          .errorhandling = "remove") %dopar% 
    {
      ...
      ID<-physeq$ID
      DD<-fit.ZICP4(features, metadata, libSize, ID, transformation, multiple_qvalues)
