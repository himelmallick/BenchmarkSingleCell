######################################################################
# Vanilla Poisson regression (gamma2) with Library Size as Covariate #
######################################################################
...
###########################
# Fit gamma2 To A Dataset #
###########################

fit.gamma2 <- function(features, 
                   metadata, 
                   libSize, 
                   ID, 
                   transformation,
  ...
  paras <- pbapply::pbsapply(1:ncol(features), simplify=FALSE, function(x){
    ...
    #######################
      ...
      fit <- tryCatch({
        fit1 <- glm(formula, data = dat_sub, family = Gamma(link = "log"))
      }, error=function(err){
        fit1 <- try({glm(formula, data = dat_sub, family = Gamma(link = "log"))}) 
        return(fit1)
      })
...
####################################
# Fit gamma2 To A List of Datasets #
####################################

list.gamma2<-function(physeq, transformation = 'NONE', multiple_qvalues = TRUE){
  foreach(physeq = physeq, 
          ...
          .errorhandling = "remove") %dopar% 
    {
      ...
      ID<-physeq$ID
      DD<-fit.gamma2(features, metadata, libSize, ID, transformation, multiple_qvalues)
